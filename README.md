...
* The users table is a table that holds user entries that contain the columns <i>user_id</i>, and <i>user</i>. The <i>user_id</i> column is the primary key and it automatically increments. The <i>user</i> column contains the name of a user that can be selected when creating an exercise log. These columns are referencing "User Data" entries.
* The exercises table is a table that holds exercise log entries that contain the columns <i>exercise_id</i>, <i>user</i>, <i>description</i>, <i>duration</i>, and <i>date</i>. The ID column is the primary key and it automatically increments, the <i>user</i> column contains the name of a selectable user, the <i>description</i> column contains the description of the exercise, the <i>duration column is how long the exercise was done for (in minutes), and the <i>date column is the date in which the exercise occurs. These columns are referencing "Exercise Log Data" entries.
